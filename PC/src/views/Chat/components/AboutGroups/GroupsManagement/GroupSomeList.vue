...
const groupMembers = computed(() => {
    return groupsStore.groupsInfos[groupDetail.value.id].members
})
//黑名单列表
const blackMemberList = computed(() => {
    return groupsStore.groupsInfos[groupDetail.value.id].blacklist
})
console.log('blackMemberList>>>>', blackMemberList.value)
//禁言列表
const muteMemberList = computed(() => {
    return groupsStore.groupsInfos[groupDetail.value.id].mutelist
})
...
const handleBlackFromSDK = (difArr) => {
    ...
    if (toBeAddList.length > 0) {
          groupsStore().addMemberToBlackList({ groupId, usernames: toBeAddList })
    }
    if (toBeRemoveList.length > 0) {
          groupsStore().removeTheMemberFromBlackList({ groupId, usernames: toBeRemoveList })
    }
    return
...
const handleMuteFromSDK = (difArr, oldMuteList) => {
    ...
    if (toBeAddList.length > 0) {
        console.log('>>>>>>添加禁言', toBeAddList)
          groupsStore().addMemberToMuteList({ groupId, usernames: toBeAddList })
    }
    if (toBeRemoveList.length > 0) {
        console.log('>>>>>移出禁言', toBeRemoveList)
          groupsStore().removeTheMemberFromMuteList({ groupId, usernames: toBeRemoveList })
    }
    return
