...
const sortedFriendList = computed(() => {
    const sourceData = _.cloneDeep(contactsStore.contacts.friendList)
...
const removeTheMember = async (params) => {
   ...
   await groupsStore().removeTheGroupMember({ username: member, groupId })
}
/* 完成操作 */
const saveHandleMembers = async () => {
    if (checkedInviteMembers.value && checkedInviteMembers.value.length) {
        const users = _.map(checkedInviteMembers.value, 'uid')
        const groupId = groupDetail.value && groupDetail.value.id
         await groupsStore().inviteUserJoinTheGroup({ users, groupId })
    }
}
