...
/* IMSDK */
/* components */
import GroupsManagement from '../GroupsManagement/index.vue'
/* icons */
...
const groupDetail = computed(() => {
    return (
        contactsStore.contacts.groupList[nowGroupId.value] &&
        contactsStore.contacts.groupList[nowGroupId.value].groupDetail
    )
})
...
const goupsInfos = computed(() => {
    return (
        ...
        groupsStore.groupsInfos[groupDetail.value.id]
    )
})
...
const editGroupName = async (type, oldGroupName) => {
    if (type === 'save') {
        ...
        console.log('>>>>>保存编辑')
        try {
            await groupsStore().modifyGroupInfo(params)
            ElMessage({
                message: '群组名称修改成功~',
                type: 'success',
                center: true
...
const editMyGroupNickName = async (type, oldMyGroupNickname) => {
    if (type === 'save') {
        ...
        console.log('>>>>>保存编辑')
        try {
            await groupsStore().setInTheGroupInfo(params)
            ElMessage({
                message: '本群昵称修改成功~',
                type: 'success',
                center: true
...
const inTheGroupNickname = computed(() => {
    const loginUser = EaseChatClient.user
    const myNickname =
        groupsStore.groupsInfos[nowGroupId.value]?.groupMemberInfo[
            loginUser
        ]?.nickName
    return myNickname
})
...
const quitThisGroup = async () => {
    ...
    try {
        await ElMessageBox.confirm(
            ...
            await groupsStore().leaveIntheGroup({ groupId })
        emits('handleDrawer')
    } catch (error) {
        if (error !== 'cancel') {
            ElMessage({
...
const dissolveThisGroup = async () => {
    ...
    try {
        await ElMessageBox.confirm(
            ...
            await groupsStore().destroyInTheGroup({ groupId })
        emits('handleDrawer')
    } catch (error) {
        if (error !== 'cancel') {
            ElMessage({
